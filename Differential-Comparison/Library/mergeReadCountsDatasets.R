...
mergeReadCountsDatasets <- function(read_counts_datasets, merge_method="intersect", missing_value=NULL, func_dir=NULL)
{
    ...
    # Merge all original experiment design field names by ensuring
    # they are identical.
    orig_exprt_design_field_names_list <- read_counts_datasets$orig_exprt_design_field_names_list
    orig_exprt_design_field_names_merged <- NULL
    for(dataset_name in names(orig_exprt_design_field_names_list))
    {
        if(is.null(orig_exprt_design_field_names_merged)) orig_exprt_design_field_names_merged <- orig_exprt_design_field_names_list[[dataset_name]]
        else stopifnot(identical(orig_exprt_design_field_names_merged, orig_exprt_design_field_names_list[[dataset_name]]))
    }
    ...
    # Replace missing values of read counts matrix.
    ...
    read_counts_datasets_merged <- list(read_counts=read_counts_merged, exprt_design=exprt_design_merged, orig_exprt_design_field_names=orig_exprt_design_field_names_merged, sample_id_fields=read_counts_datasets$sample_id_fields)
