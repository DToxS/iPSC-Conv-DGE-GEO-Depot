...
readReadCountsDatasets <- function(read_counts_files, generateReadCountsColumnNames=NULL, split_char=NULL, field_names=NULL, field_types=c(Species="character", Subject="character", State="character", Culture="character", Replicate="character", Measure="character", Sample="character", Time="character"), std_exprt_design_field_names=c("Species", "Subject", "State", "Culture", "Replicate", "Measure", "Sample", "Time"), species_default=species_default, subject_default=subject_default, state_default=state_default, culture_default=culture_default, measure_default=measure_default, time_default=time_default, row_name_column=NULL, read_counts_columns=NULL, read_counts_skip=0, exprt_design_files=NULL, field_names_positions=NULL, exprt_design_skip=0, legal_chars=NULL, repl_chars=NULL, compress_space=TRUE, func_dir=NULL)
{
    ...
    orig_exprt_design_field_names_list <- NULL
    for(idx in 1:length(read_counts_files))
    {
        ...
        if(is.null(exprt_design_files))
        {
            ...
            orig_exprt_design_field_names <- colnames(exprt_design)
        }
        else
        {
            ...
            orig_exprt_design_field_names <- colnames(exprt_design_dataset)
            exprt_design <- convertExperimentDesignTable(exprt_design=exprt_design_dataset, field_names_positions=field_names_positions, field_types=field_types, std_exprt_design_field_names=std_exprt_design_field_names, species_default=species_default, subject_default=subject_default, state_default=state_default, culture_default=culture_default, replicate_default=replicate_default, measure_default=measure_default, sample_default=1:nrow(exprt_design_dataset), time_default=time_default, legal_chars=legal_chars, repl_chars=repl_chars, compress_space=compress_space, func_dir=func_dir)
        }
        ...
        orig_exprt_design_field_names_list[[dataset_name]] <- orig_exprt_design_field_names
    }

    # Return read counts list and experiment design list.
    read_counts_datasets <- list(read_counts_list=read_counts_list, exprt_design_list=exprt_design_list, orig_exprt_design_field_names_list=orig_exprt_design_field_names_list, sample_id_fields=sample_id_fields)
