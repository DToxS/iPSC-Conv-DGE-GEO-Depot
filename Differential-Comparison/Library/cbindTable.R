...
cbindTable <- function(..., table.name=FALSE)
{
    ...
    for(tab_name in names(tables))
    {
        ...
        # rows.
        ctable_comm <- ctable[common_row_flags,, drop=FALSE]
        ctable_diff <- ctable[!common_row_flags,, drop=FALSE]
        # Sort the sub table with common rows according to the row order of current table.
        ctable_comm <- ctable_comm[order(match(rownames(ctable_comm), rownames(tab))),, drop=FALSE]
        # Splice two sub tables by rows:
        # The sub table with common rows followed by that with different rows.
        if(ncol(ctable) == 0) ctable <- data.frame(row.names=c(rownames(ctable_comm),rownames(ctable_diff)), stringsAsFactors=FALSE)
        else ctable <- rbind(ctable_comm, ctable_diff)
        # Combine reordered combined ctable table with current table and
        # fill the non-common rows with NA.
        # Note: current ctable table is always combined with the upper subset of
        # already combined table with common rows, and then the lower subset of
        # combined table with non-common rows can then be filled with NA.
        ctable <- cbindX(ctable, tab)
    }
    # Sort combined ctable table according to its row names.
    ctable <- ctable[order(rownames(ctable)),, drop=FALSE]
