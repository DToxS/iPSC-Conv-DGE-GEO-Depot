...
createDGEList <- function(read_counts, group_names=rep(1,ncol(read_counts)), ctrl_state_name=NULL, normalize=TRUE, norm_base=1e+6, remove_small=TRUE, verbose=FALSE, func_dir=NULL)
{
    ...
    if(!is.null(ctrl_state_name) && (ctrl_state_name%in%group_names))
    {
        group_factors <- factor(group_names)
        group_factors <- relevel(group_factors, ref=ctrl_state_name)
        dge_profile <- DGEList(counts=read_counts, group=group_factors)
    }
    else dge_profile <- DGEList(counts=read_counts, group=group_names)
