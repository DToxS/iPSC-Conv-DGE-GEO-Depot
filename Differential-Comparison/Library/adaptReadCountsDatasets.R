...
adaptReadCountsDatasets <- function(exprt_design, field_name_map=NULL)
{
    # field_name_map: c(Sample="Well",Culture="Experiment",Measure="Plate")

    # Initialization.
    exprt_design_mapped <- exprt_design

    # Change field names according to given name map.
    if(is.vector(field_name_map) && is.vector(names(field_name_map)))
    {
        field_names <- names(exprt_design)
        field_names_mapped <- field_name_map[field_names]
